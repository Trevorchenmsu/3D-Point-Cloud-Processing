...
class Node:
    """
    axis: axis perpendicular to splitting hyperplane
    value: store the position of splitting plane
    ...
    # since for a leaf, we don't need to split any more. so value is set as None
    def is_leaf(self): 
...
"""
    Before building a tree, we need to sort the values and corresponding keys. 
    
    input: 
            key: key is actually point indices
...
# This method is used to determine the split axis, it is not an optimal way to 
# determine the axis. A better method is to consider the variance in the axes. 
# Select the axis with larger variance, and split along the axis which is 
# perpendicular to the axis with the larger variance.
...
def kdtree_recursive_build(root, db, point_indices, axis, leaf_size):
    ...
    if len(point_indices) > leaf_size:
        ...
        # sort based on values and get the corresponding point indices
        # then use divide and conquer to split the sorted indices/values
        ...
        middle_left_point_value = db[middle_left_point_idx, axis] # db.shape: NxD
...
def kdtree_knn_search(root: Node, db: np.ndarray, result_set: KNNResultSet, query: np.ndarray):
    ...
    # reach the leaf node that covers the query points
    if root.is_leaf():
