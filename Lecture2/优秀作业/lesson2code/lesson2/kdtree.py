...
# 功能：构建树之前需要对value进行排序，同时对应的key的顺序也要跟着改变
# 输入：
#     key：键
#     value:值
# 输出：
...
def kdtree_recursive_build(root, db, point_indices, axis, leaf_size):
    ...
    if len(point_indices) > leaf_size:
        ...
        # recursive in the left branch
        root.left = kdtree_recursive_build( root.left, 
                                            db, 
                                            point_indices_sorted[0:middle_right_idx], 
                                            axis_round_robin(axis, dim=db.shape[1]), 
                                            leaf_size )
        # recursive in the right branch
        root.left = kdtree_recursive_build( root.right, 
                                            db, 
...
def main():
    ...
    print("-----Radius search kdtree:")
    query = np.asarray([0, 0, 0])
    result_set = RadiusNNResultSet(radius = 1.5)
