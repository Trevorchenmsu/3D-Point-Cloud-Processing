...
# 节点，构成OCtree的基本元素
# childern: Array of length 8
# center: center of the cube
# extent: 0.5 * length
# point_indices: point inside octant
class Octant:
...
def octree_recursive_build(root, db, center, extent, point_indices, leaf_size, min_extent):
    ...
    if len(point_indices) <= leaf_size or extent <= min_extent:
        ...
        root.is_leaf = False
        # 新开辟一个8个格子的Cube空间
        child_point_indices = [ [] for i in range(8) ]
        for point_idx in point_indices:
            point_db = db[point_idx] # 取数据：点point_db
            morton_code = 0 # 属于[0,7]
            # 看该point是属于哪个child
            # x | y Does a "bitwise or". 
            # Each bit of the output is 0 if the corresponding bit of x AND of y is 0, otherwise it's 1
            if point_db[0] > center[0]:         # 比较第一维x
                morton_code = morton_code | 1   # 000｜001 = 001 相当于+1
            if point_db[1] > center[1]:         # y
                morton_code = morton_code | 2   # 000｜010 = 010 or 001｜010 = 011 相当于+2
            if point_db[2] > center[2]:         # z
                morton_code = morton_code | 4   # 000｜100 = 100 or 010｜100 = 110 or 011｜100 = 111 相当于+4
            # 将上面开辟的Cube空间划分成第0-7共8块，把各个点point_db的key放入对应的小cube(Octant)中
            child_point_indices[morton_code].append(point_idx) 
        ...
        for i in range(8):
            ...
            child_center_x = center[0] + factor[(i & 1) > 0] * extent # i = 1,3,5,7, 说明上面分块执行过|1，(i & 1) = 1 > 0成立，即取factor[1] = 0.5;否则取factor[1] = -0.5
            child_center_y = center[1] + factor[(i & 2) > 0] * extent # i = 2,3,6,7, 说明上面分块执行了|2，(i & 2) = 2 > 0成立，即取factor[1] = 0.5;否则取factor[1] = -0.5
            child_center_z = center[2] + factor[(i & 4) > 0] * extent # i = 4,5,6,7, 说明上面分块执行了|4，(i & 4) = 4 > 0成立，即取factor[1] = 0.5;否则取factor[1] = -0.5
            child_extent = 0.5 * extent # 对半切
            child_center = np.asarray([child_center_x, child_center_y, child_center_z]) # 递归
            root.children[i] = octree_recursive_build(root.children[i], db, child_center, child_extent, child_point_indices[i], leaf_size, min_extent)
        # 屏蔽结束
    return root
...
def inside(query: np.ndarray, radius: float, octant:Octant):
    ...
    query_offset_abs = np.fabs(query_offset) # check point to center
...
def overlaps(query: np.ndarray, radius: float, octant:Octant):
    ...
    max_dist = radius + octant.extent #* 1.732 
    if np.any(query_offset_abs > max_dist): #不相交
        return False

    # if pass the above check, consider the case that the ball is contacting the face of the octant
    if np.sum((query_offset_abs < octant.extent).astype(np.int)) >= 2: #球与面相交
        return True
    ...
    return x_diff * x_diff + y_diff * y_diff + z_diff * z_diff < radius * radius #是否与楞角相交
...
def contains(query: np.ndarray, radius: float, octant:Octant):
    ...
    query_offset_to_farthest_corner = query_offset_abs + octant.extent # *1.732
...
def octree_radius_search_fast(root: Octant, db: np.ndarray, result_set: RadiusNNResultSet, query: np.ndarray):
    ...
    for c, child in enumerate(root.children):
        if child is None: # 没child
            continue
        if False == overlaps(query, result_set.worstDist(), child): # 不相交，跳过
            continue
        if octree_radius_search_fast(child, db, result_set, query): # 递归
            return True
...
def octree_radius_search(root: Octant, db: np.ndarray, result_set: RadiusNNResultSet, query: np.ndarray):
    ...
    # search the most relevant child
    if octree_radius_search(root.children[morton_code], db, result_set, query): 
        return True
    
    # check other children
    for c, child in enumerate(root.children):
        if c == morton_code or child is None: # 上面check过，或者没child
            continue
        if False == overlaps(query, result_set.worstDist(), child): # 不相交，跳过
            continue
        if octree_radius_search(child, db, result_set, query): # 递归
            return True
...
def octree_knn_search(root: Octant, db: np.ndarray, result_set: KNNResultSet, query: np.ndarray):
    ...
    for c, child in enumerate(root.children):
        ...
        if False == overlaps(query, result_set.worstDist(), child): # 不相交，跳过
            continue
        if octree_knn_search(child, db, result_set, query): # 递归
            return True
    # 屏蔽结束
